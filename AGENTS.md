...
- Certification badges should open a themed solid detail popup with metadata and a low-profile verification link instead of navigating directly; selection should use a smooth camera-style zoom that moves the original selected SVG badge into the popup header area, then visibly draws the popup frame around it without an abrupt cut, halo, translucent panel, duplicate badge icon, or hover-driven grey/refocus pulsing.
