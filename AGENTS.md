...
- Certification badges should open a themed detail popup with metadata and a low-profile verification link instead of navigating directly; selection should use a smooth camera-style zoom into the constellation, then hand off from the original zoomed badge to a snappy drop-in badge at the top of the popup above the text without simultaneous duplicate icons.
